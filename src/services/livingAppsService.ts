...
import type { Dateneingabe, CreateDateneingabe } from '@/types/app';
...
export class LivingAppsService {
  // --- DATENEINGABE ---
  static async getDateneingabe(): Promise<Dateneingabe[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.DATENEINGABE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Dateneingabe[];
    return enrichLookupFields(records, 'dateneingabe');
  }
  static async getDateneingabeEntry(id: string): Promise<Dateneingabe | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.DATENEINGABE}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Dateneingabe;
    return enrichLookupFields([record], 'dateneingabe')[0];
  }
  static async createDateneingabeEntry(fields: CreateDateneingabe): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.DATENEINGABE}/records`, { fields: cleanFieldsForApi(fields as any, 'dateneingabe') });
    return { ...data, record_id: data.id };
  }
  static async updateDateneingabeEntry(id: string, fields: Partial<CreateDateneingabe>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.DATENEINGABE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'dateneingabe') });
    return { ...data, record_id: data.id };
  }
  static async deleteDateneingabeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.DATENEINGABE}/records/${id}`);
  }
