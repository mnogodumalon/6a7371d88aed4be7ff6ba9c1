    ...
    <div className="mt-1.5 w-full max-w-[85%] rounded-xl border border-border bg-card px-3.5 py-2.5">
      <div className="flex items-center gap-2">
        ...
        {action ? (
          <button
            type="button"
            title={`${info.actionName} — Open action`}
            onClick={() => devMode
...
export function RunIdChip({ runId, className = '' }: { runId: string; className?: string }) {
  ...
  return (
    <button
      type="button"
      title="Copy RunID — quote it when reporting a problem"
      onClick={() => {
          ...
          <div key={`${group.key}-${group.sessions[0].id}`}>
            ...
            {group.sessions.map(s => (
              <div key={s.id} className="group relative">
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="flex items-baseline gap-2">
                      <span className="min-w-0 flex-1 truncate text-sm font-medium text-foreground">{stripMd(s.ai?.title || s.title) || 'Assistant'}</span>
                    ...
                    {(s.id === activeThreadId || s.origin === 'fix' || !!s.action || (!!s.user?.initials && !s.mine)) && (
                      <span className="mt-1 flex flex-wrap items-center gap-1">
                        {s.id === activeThreadId && (
                          <span className="inline-flex items-center rounded-full border border-green-200 bg-green-50 px-1.5 py-px text-[10px] font-semibold text-green-700">Active</span>
                        )}
                        {s.origin === 'fix' && (
                ...
                >
                  {armedDelete === s.id ? <span className="px-0.5 text-[11px] font-semibold">Delete?</span> : <IconTrash size={13} />}
                </button>
              </div>
            ))}
          </div>
...
export function ChatPanel({ placeholder = 'Ask a question or upload an image...', autoFocus = false, collapsed = false }: { placeholder?: string; autoFocus?: boolean; collapsed?: boolean }) {
      ...
      <div ref={scrollRef} className={collapsed ? 'hidden' : 'flex-1 overflow-y-auto px-4 py-3 space-y-3'}>
        {scopedFresh ? (
          <div className="flex flex-col items-center justify-center h-full text-center gap-2 text-muted-foreground">
            ...
            <p className="text-xs max-w-[280px]">Ask about this action — the answer knows its code and versions.</p>
            <div className="mt-1 flex flex-wrap justify-center gap-1.5">
              <button
                type="button"
                onClick={() => sendMessage('What does this action do?')}
              ...
              <button
                type="button"
                onClick={() => sendMessage('Explain the code to me')}
                className="rounded-full border border-primary/40 bg-card px-3 py-1.5 text-xs font-medium text-primary hover:bg-primary/10 transition-colors"
              >
        ...
        {codeDrawerAction && dockScope === 'global' && !sessionMatchesDrawer && (
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1 rounded-xl border border-dashed border-border bg-muted/60 px-3 py-2 text-xs text-muted-foreground">
            <span>
              {sessionAction
                ? <>This conversation belongs to <span className="font-medium text-foreground">„{sessionAction.title || sessionAction.identifier}"</span></>
                : 'General conversation, not tied to an action'}
            </span>
            <button type="button" onClick={() => setDockScope('action')} className="font-semibold text-primary hover:underline">
              New conversation about this action
            </button>
          </div>
        )}
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center gap-2 text-muted-foreground">
            ...
            {chatSessions.filter(s => s.id !== activeThreadId).length > 0 && (
              <div className="mt-3 w-full max-w-[260px] text-left">
                <p className="mb-1.5 px-0.5 text-[10px] font-bold uppercase tracking-wider text-muted-foreground/70">Recent</p>
        ...
        {resumedSessionAt !== null && messages.length > 0 && (
          <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
            ...
            <span className="shrink-0">
              Session resumed{sessionTime(resumedSessionAt) ? ` · ${sessionTime(resumedSessionAt)}` : ''}
            </span>
            <span className="h-px flex-1 bg-border" />
          </div>
        )}
        {messages.map((m) => (
          ...
          <div key={m.id} className={`group flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
            ...
            {m.fixContext && (
              ...
              >
                <IconWand size={16} />
                {fixingMessageId === m.id ? 'Fixing…' : 'Try to fix'}
              </button>
            )}
            {m.versionInfo && (
