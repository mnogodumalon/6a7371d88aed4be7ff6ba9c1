...
function versionSummary(v: ActionVersion): string {
  ...
  if (v.origin === 'revert' && v.revert_of) return `Restored to version ${v.revert_of}`;
...
export function ActionCodeDrawer() {
  ...
  const handleRestore = useCallback(async () => {
    ...
    const ok = window.confirm(`Restore version v${selectedEntry.v}?\n\nThe current code will be replaced. Nothing is lost — a new version is created.`);
    if (!ok) return;
    setRestoring(true);
    try {
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <nav aria-label="Versions" className="hidden lg:flex w-64 shrink-0 flex-col gap-0.5 overflow-y-auto border-r border-border px-3 py-3">
            ...
            {versions !== null && newestFirst.length === 0 && (
              <p className="px-2 text-xs text-muted-foreground">No previous versions</p>
            )}
            {newestFirst.map(v => (
          ...
          <div className="flex flex-1 min-w-0 flex-col">
            ...
            <div className="flex items-center gap-1.5 border-b border-border px-3 py-2 shrink-0">
              ...
              >
                ...
                {isRunning ? 'Working...' : isOld ? `Test v${selected}` : 'Run'}
              </button>
            </div>
            ...
            {isOld && (
              <div className="flex flex-wrap items-center gap-2 border-b border-border bg-secondary px-3 py-2 text-xs shrink-0">
                <span>
                  Viewing <b>v{selected}</b> ({formatDateTime(selectedEntry?.ts)}) — not the active version.
                </span>
                <span className="flex-1" />
                <button
                  type="button"
              ...
              <div className="flex-1 min-h-0 overflow-auto px-4 py-3 space-y-3">
                <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                  <span className="text-sm font-semibold text-foreground">
                    Test run{run.version != null ? ` v${run.version}` : (current > 0 ? ` v${current}` : '')}
                  </span>
                  <span className="tabular-nums">{formatDateTime(new Date(run.ts).toISOString())}</span>
                  {run.version != null && (
                    <span className="rounded-full bg-secondary px-2 py-0.5 text-[11px] font-medium">
                      Code from history — not restored
                    </span>
                  )}
            ...
            <div className="flex items-center gap-3 border-t border-border px-4 py-1.5 text-[11px] text-muted-foreground tabular-nums shrink-0">
              ...
              <span>v{selectedEntry?.v ?? current}{isOld ? '' : ` · Active`}</span>
        ...
        <div className={`relative shrink-0 border-t border-border bg-card flex flex-col ${dockOpen ? 'h-[45dvh] min-h-[15rem]' : ''}`}>
          ...
          <div className="relative flex shrink-0 items-center gap-1 px-2">
            ...
            <button
              ...
              title={dockScope === 'action' ? (action ? action.title || action.identifier : '') : 'General conversation'}
            ...
            >
              ...
              <span className="min-w-0 truncate">{dockScope === 'action' ? (action ? action.title || action.identifier : '') : 'General'}</span>
            ...
            <div className="flex shrink-0 items-center gap-0.5">
              <button
                ...
                title="New conversation about this action"
              ...
              <div className="absolute bottom-full left-2 z-10 mb-2 w-80 max-w-[calc(100vw-2rem)] overflow-hidden rounded-2xl border border-border bg-card p-1.5 shadow-2xl">
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block text-xs text-muted-foreground">Questions & changes about this action</span>
                  </span>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-medium text-foreground">General conversation</span>
                    {sessionAction && !sessionMatchesAction && (
                      <span className="block truncate text-xs text-muted-foreground">Last: {sessionAction.title || sessionAction.identifier}</span>
                    )}
                  </span>
                </button>
              </div>
          ...
          <ChatPanel placeholder="Ask about this code…" collapsed={!dockOpen} />
        </div>
      </aside>
    </>,
    document.body
