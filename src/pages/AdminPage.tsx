...
import { DateneingabeViewDialog } from '@/components/dialogs/DateneingabeViewDialog';
...
const ENTITY_TABS = [
  { key: 'dateneingabe', label: 'Dateneingabe', pascal: 'Dateneingabe' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('dateneingabe');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'dateneingabe': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'dateneingabe': return (data as any).dateneingabe as Dateneingabe[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'dateneingabe': return DATENEINGABE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'dateneingabe': return {
        create: (fields: any) => LivingAppsService.createDateneingabeEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateDateneingabeEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteDateneingabeEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'dateneingabe' || dialogState?.entity === 'dateneingabe') && (
        <DateneingabeDialog
          open={createEntity === 'dateneingabe' || dialogState?.entity === 'dateneingabe'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'dateneingabe' ? handleUpdate : (fields: any) => handleCreate('dateneingabe', fields)}
          defaultValues={dialogState?.entity === 'dateneingabe' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Dateneingabe']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Dateneingabe']}
        />
      )}
      {viewState?.entity === 'dateneingabe' && (
        <DateneingabeViewDialog
          open={viewState?.entity === 'dateneingabe'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'dateneingabe', record: r }); }}
        />
      )}
