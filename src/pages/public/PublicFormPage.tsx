  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    for (const field of page.fields) {
      if (field.required && isEmpty(values[field.key])) {
        missing[field.key] = 'This field is required.';
      }
    }
    ...
    try {
      ...
      if (err instanceof FieldValidationError) {
        ...
        for (const key of err.missingFields) errs[key] = 'This field is required.';
        setFieldErrors(errs);
  ...
  if (status === 'unavailable' || !page || !config) {
    return shell(
      <div className="rounded-[27px] bg-card shadow-lg p-6 sm:p-8 text-center">
        <h1 className="text-xl font-medium mb-2">Not available</h1>
        <p className="text-muted-foreground">This form is currently not available.</p>
      </div>,
    );
  }
  ...
  if (status === 'done') {
    return shell(
      <div className="rounded-[27px] bg-card shadow-lg p-6 sm:p-8 text-center">
        ...
        <Button variant="outline" onClick={resetForAnotherEntry}>Submit another</Button>
      </div>,
    );
  }
