...
export default function DateneingabePage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<Dateneingabe[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Dateneingabe | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getDateneingabe());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Dateneingabe['fields']) {
  ...
  async function handleUpdate(fields: Dateneingabe['fields']) {
    ...
    await LivingAppsService.updateDateneingabeEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Dateneingabe"
      subtitle={`${records.length} Dateneingabe in the system`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Search Dateneingabe..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/dateneingabe/${record.record_id}`); }}>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="text-center py-16 text-muted-foreground">
                  {search ? 'No results found.' : 'No Dateneingabe yet. Add one!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
