...
export function ActionsProvider({ children }: { children: ReactNode }) {
  ...
  const executeAndReport = useCallback((action: Action, inputs?: Record<string, unknown>, files?: File[], version?: number, silent = false) => {
    ...
    if (!silent) {
      setMessages(prev => [
        ...prev,
        { id: crypto.randomUUID(), role: 'user', kind: 'action', content: `Action: ${action.title || action.identifier}${version != null ? ` (v${version})` : ''}` },
    ...
    executeAction(action.app_id, action.identifier, inputs, files, version)
      ...
      .catch(err => {
        ...
        setMessages(prev =>
          prev.map(m =>
            m.id === placeholderId
              ? { ...m, content: `Execution failed: ${msg}` }
              : m,
          )
        );
      })
    ...
    if (schema['x-preflight']) {
      ...
      executeAction(action.app_id, action.identifier, {}, undefined, version)
        ...
        .catch(err => {
          ...
          setMessages(prev => [
            ...prev,
            { id: crypto.randomUUID(), role: 'assistant', content: `Execution failed: ${msg}` },
          ]);
        })
        .finally(() => {
          chatLoadingRef.current = false;
  ...
  const revertActionVersion = useCallback(async (appId: string, identifier: string, to: number, expectedCurrent?: number) => {
    ...
    if (!result.ok || !result.version) {
      ...
      setMessages(prev => [
        ...prev,
        { id: crypto.randomUUID(), role: 'assistant', content: `**Restore failed:** ${result.error ?? ''}` },
      ]);
      return;
    }
    appendVersionCard({
      ...
      summary: `Restored to version ${to}`,
      origin: 'revert',
    });
    void refreshActions();
    window.dispatchEvent(new Event('dashboard-refresh'));
    ...
    setMessages([
      {
        id: crypto.randomUUID(),
        role: 'assistant',
        content: `**Fixing \`${ctx.actionName}\`** — started a new chat session for this fix.\n\`\`\`\n${ctx.errorText}\n\`\`\``,
      },
    ...
    try {
      ...
      if (result?.success) {
        ...
        const note: Message = {
          ...
          content: result?.error
            ...
            : '*The fix is not confirmed yet — your original input stays preserved.*',
      ...
      setMessages(prev => [
        ...prev,
        {
          id: crypto.randomUUID(),
          role: 'assistant',
          content: `**Fix request failed:** ${err instanceof Error ? err.message : String(err)}\n\n*Your original input stays preserved — you can retry the fix.*`,
          fixContext: ctx,
        },
      ]);
    } finally {
